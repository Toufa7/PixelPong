...
const UpdateGroup = (id : string) => {
	...
	console.log("Avatae Sended is -> ", groupAvatar);
		...
		<div className="groupSettings" style={{display: 'flex',flexDirection: 'column',justifyContent: 'center',alignItems: 'center'}}>
			<div style={{display: 'flex', justifyContent: "space-evenly"}}>
			{
				groupsList.map((group : any) => (
					<a>
						<img onClick={() => setSelecting(group)} src={`http://localhost:3000/groupchat/getimage/${group.id}`} title={group.namegb} style={{ borderRadius: '50%', width: '60px', height: '60px', margin: '10px' }} alt="avatar" />
						</a>
					))
				}
			</div>
